...
| Dodavatelé – trvalé příkazy (TP / PR) | JHN-PROJECT `dbo.Salda_TrvalePrikazy` | rozepisují se v prohlížeči podle frekvence (týdenní, 14 dní, měsíční, čtvrtletní, pololetní, roční, jednorázově) do období, počínaje dneškem; záporná částka = pravidelný příjem |
...
- `sql/salda.sql` – DDL tabulky `dbo.Salda_TrvalePrikazy` v **JHN-PROJECT** + jednorázový seed 45 záznamů
...
2. Spustit `sql/salda.sql` na databázi **JHN-PROJECT** (např. v SQL Studiu, spojení `jhn`) – založí tabulku a naplní ji.
