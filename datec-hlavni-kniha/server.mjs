...
const server = http.createServer(async (req, res) => {
  try {
    ...
    else { console.error('[kniha]', e); res.writeHead(500, { 'Content-Type': 'text/plain; charset=utf-8' }); res.end('Chyba serveru'); }
  }
});

const port = Number(process.env.PORT || 3092);
const host = process.env.HOST || '127.0.0.1';
server.listen(port, host, () => console.log(`Hlavní kniha běží na http://${host}:${port}  (SQL ${process.env.SQL_SERVER || '?'}: Helios005 + Helios004)`));
